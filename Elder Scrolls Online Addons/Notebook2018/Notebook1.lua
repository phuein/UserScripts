...
function Populate_NB1_ScrollList(noPageSelect)
	...
	for i = 1, numPages do
		...
		local title = button.data.title
		title = NBUI.NoTagsText(title)

		if title == '---' then
			button:SetText('|t400:2:esoui/art/miscellaneous/horizontaldivider.dds|t')
			...
			button:SetText(NBUI.UnprotectText(title))
		end
	...
	-- Remove unsaved pages from pool.
	...
	if not noPageSelect and numPages > 0 then
		...
		if currentlyViewing then
			-- Stay on current page, it was just saved.
			last = currentlyViewing
		else
			-- Last page was deleted, so go one back.
			if last - 1 == numPages then
				last = last - 1
			end
		end

		-- Get the button object.
		local po = NB1_IndexPool:AcquireObject(last)

		-- At least one page is available.
		if po then
			-- Display it.
			zo_callHandler(po, "OnClicked")
		end
	end
end
---------------------------------------------------------------------------------------------------
...
---------------------------------------------------------------------------------------------------
	NBUI.NB1Information_Button = WINDOW_MANAGER:CreateControl("NBUI_NB1Information_Button", NBUI.NB1MainWindow, CT_BUTTON)
		...
		NBUI.NB1Information_Button:SetHandler("OnMouseEnter", function(self)
...
---------------------------------------------------------------------------------------------------
	NBUI.NB1LeftPage_Backdrop = WINDOW_MANAGER:CreateControlFromVirtual("NBUI_NB1LeftPage_Backdrop", NBUI.NB1MainWindow, "ZO_EditBackdrop")
		...
		NBUI.NB1LeftPage_Backdrop:SetAnchor(TOPLEFT, NBUI.NB1MainWindow_Cover, TOPLEFT, 90, top)
		NBUI.NB1LeftPage_Backdrop:SetCenterColor(0, 0, 0, 0)
		NBUI.NB1LeftPage_Backdrop:SetDimensions(420, height)
...
---------------------------------------------------------------------------------------------------
	NBUI.NB1MovePageUp_Button = WINDOW_MANAGER:CreateControl("NBUI_NB1MovePageUp_Button", NBUI.NB1LeftPage_ScrollContainer.scrollChild, CT_BUTTON)
		NBUI.NB1MovePageUp_Button:SetAnchor(RIGHT, NBUI.NB1SelectedPage_Button, RIGHT, -96, 0)
		NBUI.NB1MovePageUp_Button:SetClickSound(SOUNDS.BOOK_PAGE_TURN)
		NBUI.NB1MovePageUp_Button:SetDimensions(20, 20)
		NBUI.NB1MovePageUp_Button:SetDrawLayer(2)
...
---------------------------------------------------------------------------------------------------
	NBUI.NB1MovePageDown_Button = WINDOW_MANAGER:CreateControl("NBUI_NB1MovePageDown_Button", NBUI.NB1LeftPage_ScrollContainer.scrollChild, CT_BUTTON)
		NBUI.NB1MovePageDown_Button:SetAnchor(RIGHT, NBUI.NB1SelectedPage_Button, RIGHT, -66, 0)
		NBUI.NB1MovePageDown_Button:SetClickSound(SOUNDS.BOOK_PAGE_TURN)
		NBUI.NB1MovePageDown_Button:SetDimensions(20, 20)
		NBUI.NB1MovePageDown_Button:SetDrawLayer(2)
...
---------------------------------------------------------------------------------------------------
	...
	-- 	NBUI.NB1UndoPage_Button:SetDrawLayer(2)
...
---------------------------------------------------------------------------------------------------
	...
	-- 	NBUI.NB1RunScript_Button:SetDrawLayer(2)
...
---------------------------------------------------------------------------------------------------
	NBUI.NB1RightPage_Title = WINDOW_MANAGER:CreateControlFromVirtual("NBUI_NB1RightPage_Title", NBUI.NB1RightPage_TitleBackdrop, "ZO_DefaultEditForBackdrop")
		...
		NBUI.NB1RightPage_Title:SetMaxInputChars(MAIL_MAX_SUBJECT_CHARACTERS)
...
---------------------------------------------------------------------------------------------------
	NBUI.NB1RightPage_Contents = WINDOW_MANAGER:CreateControlFromVirtual("NBUI_NB1RightPage_Contents", NBUI.NB1RightPage_ScrollContainer, "ZO_DefaultEditMultiLineForBackdrop")
		...
		NBUI.NB1RightPage_Contents:SetMaxInputChars(3000) -- Visual max, higher than saveable max. -- MAIL_MAX_BODY_CHARACTERS
...
function NBUI.NB1SavePage(self)
	...
	local safe_titleText = NBUI.ProtectText(titleText)
	local safe_pageText = NBUI.ProtectText(pageText)
