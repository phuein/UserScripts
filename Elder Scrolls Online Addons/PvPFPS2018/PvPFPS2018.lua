...
-- Returns current SubSampling level.
-- Repeated use in functions will set their own curSS variable locally!
local function curSS()
    return GetSetting(SETTING_TYPE_GRAPHICS, GRAPHICS_SETTING_SUB_SAMPLING)
end
...
local function changeSS(v)
    ...
    d('Changed SubSampling from '..curSS..' to '..v..'.')
end
