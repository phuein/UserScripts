...
import time, codecs, re, random, signal, sys
...
def scrape(page):
    try:
        ...
        DEBUG and print('Failed to connect: '+str(err))
        return []
    ...
    # itemprop=\"price\"\u003e$149+\u003c
    p = 'itemprop=\\"price\\"\\u003e'
    # class=\"discover-product__information\"\u003e\n\u003ch2\u003e\nvoxel plugin™ PRO\n\u003c/h2
    t = 'class=\\"discover-product__information\\"\\u003e\\n\\u003ch2\\u003e\\n'
...
def run():
    ...
    for i in range(maxResults):
        ...
        link = '/discover_search?from=' + str(v) + '&query=' + quote(query)

        # Imitate human delay.
        if i > 1:
            # 1-3 seconds.
            z = random.randrange(10, 30) / 10
            DEBUG and print('Page '+str(i)+' - Sleeping for '+str(z)+' seconds...')
            time.sleep(z)
